...
В рамках предмета мы будем работать с одним репозиторием [cpp_labs](https://github.com/Egor0212/cpp_labs.git). Каждому студенту нужно сделать ответвление от этого репозитория к себе на свой аккаунт github (так назыавемый **fork**), с которым он будет работать и загружать результаты своей проделанной работы. Подробнее как это сделать будет разобрано на занятии.
